...
namespace Application.UseCases.UseCaseProveedor
{
    public class CrearProveedor
    {
        ...
        private async Task ValidarProveedor(Proveedor proveedor)
        {
            ...
            bool existe = await _proveedor.ExistePorNombre(proveedor.Nombre.Trim());
            if (existe)
                throw new ArgumentException($"Ya existe un proveedor con el nombre '{proveedor.Nombre}'.");
        }
    }
}
